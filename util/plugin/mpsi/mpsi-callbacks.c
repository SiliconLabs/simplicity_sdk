...
uint8_t mpsiHandleMessageGetAppsInfo(MpsiMessage_t* mpsiMessage)
{
  ...
  while (SLOT_MANAGER_SUCCESS
         == sl_util_af_slot_manager_get_slot_info(slot, &slotInfo)) {
...
#endif // EMBER_AF_PLUGIN_SLOT_MANAGER
  return sl_util_af_mpsi_send_message(&response);
}
...
uint8_t mpsiHandleMessageBootloadSlot(MpsiMessage_t* mpsiMessage)
{
  ...
  status = sl_util_af_slot_manager_verify_and_bootload_slot(message.slotId);
...
uint8_t mpsiHandleMessageInitiateJoining(MpsiMessage_t* mpsiMessage)
{
  ...
  sl_status_t status = emberAfGetNodeType(&nodeType);
  if (SL_STATUS_OK == status) {
    emberAfPermitJoin(MPSI_PERMIT_JOIN_TIME_SEC, false); // no broadcast
  } else if (SL_STATUS_NOT_JOINED == status) {
#ifdef EMBER_AF_API_NETWORK_STEERING
...
uint8_t mpsiHandleMessageSetZigbeeJoiningDeviceInfo(MpsiMessage_t* mpsiMessage)
{
  ...
  status = addTransientLinkKey(message.eui64, (sl_zigbee_sec_man_key_t*)&key);
  if (SL_STATUS_OK != status) {
  ...
  MpsiMessage_t* mpsiMessage)
{
  ...
  sl_zigbee_key_data_t key;
  uint8_t i;
  sl_802154_long_addr_t wildcardEui64 = WILDCARD_EUI64;
  ...
  if (SL_STATUS_OK != status) {
    return (SL_STATUS_NOT_JOINED == status) ? MPSI_INVALID_FUNCTION : MPSI_ERROR;
  }
  ...
  memcpy(tcJoiningCredentialsMessage.extendedPanId,
         parameters.extendedPanId,
         COUNTOF(tcJoiningCredentialsMessage.extendedPanId));
  memcpy(tcJoiningCredentialsMessage.preconfiguredKey,
         key.contents,
  ...
  status = addTransientLinkKey(wildcardEui64, (sl_zigbee_sec_man_key_t*)&key);
  if (SL_STATUS_OK != status) {
    mpsiPrintln("MPSI (0x%x) error: failed to update key table (0x%x)",
                mpsiMessage->messageId, status);
    return MPSI_ERROR;
                             ...
                             void *context)
{
  ...
  if (0 == gGblMetadataBufferIndex) {
    ...
    if (memcmp(MPSI_GBL_METADATA_MAX_MESSAGE_KEY, data, minLength) == 0) {
      if (length > sizeof(gGblMetadataBuffer)) {
        length = sizeof(gGblMetadataBuffer);
      }
      memcpy(gGblMetadataBuffer, data, length);
    ...
    memcpy(gGblMetadataBuffer + gGblMetadataBufferIndex, data, length);
    gGblMetadataBufferIndex += length;
  }
...
uint16_t getMaxMessageIdSupportedBySlot(uint32_t slotId, uint32_t capabilities)
{
  ...
  (void)sl_util_af_slot_manager_get_metadata_tags_from_gbl(slotId,
