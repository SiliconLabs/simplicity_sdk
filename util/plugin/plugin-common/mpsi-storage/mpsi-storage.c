...
void sl_util_af_mpsi_storage_init_cb(void)
{
  ...
  if (true == verifyAllMpsiStorageMessagesSupported()) {
    ...
    while (bg_err_success == errorCode) {
      ...
      bytesDeserialized = sl_util_af_mpsi_deserialize(value, &mpsiMessage);

      if (bytesDeserialized) {
        // Process the MPSI message
        sl_util_af_mpsi_receive_message(&mpsiMessage);
      } else {
...
uint8_t sli_mpsi_storage_store_message(MpsiMessage_t* mpsiMessage)
{
  ...
  bytesSerialized = sl_util_af_mpsi_serialize(mpsiMessage, payload);
