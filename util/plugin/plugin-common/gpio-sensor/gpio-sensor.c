...
void sl_util_af_gpio_sensor_state_changed_cb(uint8_t);
...
void sl_util_af_gpio_sensor_interrupt_event_handler(sli_zigbee_event_t* event)
{
  ...
  emberEventSetInactive(sl_util_af_gpio_sensor_interrupt_event);
...
{
  emberEventSetInactive(sl_util_af_gpio_sensor_debounce_event);
  lastSensorStatus = newSensorStatus;

  sl_util_af_gpio_sensor_state_changed_cb(newSensorStatus);
}
