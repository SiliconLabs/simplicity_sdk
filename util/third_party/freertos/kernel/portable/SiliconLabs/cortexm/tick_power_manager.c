...
#if defined(SL_CATALOG_POWER_MANAGER_ARM_SLEEP_ON_EXIT_PRESENT)
  // Init the KERNEL1 software interrupt for the sleep-on-exit feature
  sl_interrupt_manager_set_irq_handler(KERNEL1_IRQn, sl_freertos_pend_wfi_isr_handler);
  sl_interrupt_manager_set_irq_priority(KERNEL1_IRQn,
                                        sl_interrupt_manager_get_lowest_priority());
  sl_interrupt_manager_enable_irq(KERNEL1_IRQn);
#endif
}
...
#if defined(SL_CATALOG_POWER_MANAGER_ARM_SLEEP_ON_EXIT_PRESENT)
  // When using Sleep on Exit, instead of calling __WFI(), we go into sleep
  // with the KERNEL1 irq.
  // This is because the sleep-on-exit logic is in the interrupt exit hook.
  // This will prevent race conditions where a task becomes ready just
  // when we go into sleep.
  // The __ISB() is here just to make sure we jump to the interrupt before
  // continuing.
  sl_interrupt_manager_set_irq_pending(KERNEL1_IRQn);
  __ISB();
#else
...
#if defined(SL_CATALOG_POWER_MANAGER_ARM_SLEEP_ON_EXIT_PRESENT)
/***************************************************************************//**
 * Interrupt routine called when we want to pend a WFI instruction.
 *
 * This handler doesn't do much other than calling the presleep and clearing
 * it's own pending bit.
 * This is because the sleep-on-exit logic is in the interrupt exit hook.
...
void sl_freertos_pend_wfi_isr_handler()
{
  ...
  sl_interrupt_manager_clear_irq_pending(KERNEL1_IRQn);
}
#endif
