...
static void sx_hash_free(struct sxhash *c)
{
...
static void sx_hash_pad(struct sxhash *c)
{
    ...
    ADD_INDESC_PRIV_RAW(c->dma, OFFSET_EXTRAMEM(c),
        padsz, c->dmatags->data);
    c->feedsz += padsz;
    /* Padding is done only when sx_hash_resume_state() has been called and
     * sx_hash_resume_state() already reserved room for this extra descriptor.
     * */
}
...
{
    if (c->totalfeedsz == 0) {
        /* If we get here it means an empty message hash will be generated.
         * 4 dummy bytes will be added to hash, these bytes will be dropped by
         * the hash engine. */
...
static int sx_hash_create_ba413(struct sxhash *c, size_t csz)
{
     ...
     * last 7 bits of c->algo->cfgword */
    hwreqs = (c->algo->cfgword & REG_BA413_CAPS_ALGO_MASK);
...
int sx_hash_create(struct sxhash *c, const struct sxhashalg *alg,
    size_t csz)
{
    c->algo = alg;
...
{
    if (!c->algo || c->dma.regs)
        return SX_ERR_UNITIALIZED_OBJ;
    size_t totalsz = c->totalfeedsz;
    int r = c->algo->reservehw(c, sizeof(*c));
    if (r != SX_OK)
        return r;
    c->totalfeedsz = totalsz;
    ADD_INDESC_PRIV(c->dma,
        (OFFSET_EXTRAMEM(c) + sizeof(c->extramem) - c->algo->statesz),
...
int sx_hash_status(struct sxhash *c)
{
    ...
    sx_flush_fromhw(c->dma.regs,
        (char*)&c->extramem + sizeof(c->extramem) - c->algo->statesz,
        OFFSET_EXTRAMEM(c) + sizeof(c->extramem) - c->algo->statesz,
        c->algo->statesz);
...
void sx_hash_abandon(struct sxhash *c)
{
    if (c->dma.regs)
        sx_hash_free(c);
}
