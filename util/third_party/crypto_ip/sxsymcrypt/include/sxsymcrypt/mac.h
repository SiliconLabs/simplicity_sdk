...
/** Feeds multiple data to be used for MAC generation.
 *
 * The function can be used when data is scattered in memory.
 * The function will return immediately.
 *
 * In order to start the operation sx_mac_generate() must be called.
 *
 * @param[in,out] c MAC operation context
 * @param[in] datain head of the array containing input buffers to be encrypted
 ...
 * @pre - sx_mac_create_*() function must be called first
 *
 * @remark - this function can be called even if data size, \p countdatain, is 0.
 * @remark - this function can be called only once or multiple times before invoking the
 *           hardware. In any case, the total number of descriptors should be less or equal
 *           to SX_MAC_EXTRA_DESC_IN + 1.
 */
