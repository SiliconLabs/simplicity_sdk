 ...
 */
#define SX_AES_DECRYPT 1
#define SX_AES_ENCRYPT 0

/** AES configurations.
 * Misc parameters that can be passed to sx_aead_create_aes_generic(),
 * sx_blkcipher_create_aes_generic() and sx_mac_create_aes_generic().
 */
struct sxaesparams {
    uint32_t config;
};
 ...
 * and sx_mac_create_aes_generic().
 */
#define SX_AES_UNSET_TEMPO_REDUNDANCY(config) ((config) &= (~1u))
...
int sx_blkcipher_create_aesofb_dec(struct sxblkcipher *c,
 ...
 /** Prepares an AES block cipher operation
 *
 * This function initializes the user allocated object \p c with a new block
 * cipher operation context needed to run AES CBC, ECB, OFB, CFB, CTR
 * and XTS modes and reserves the HW resource.
 *
 * After successful execution of this function, the context \p c can be passed
 * to any of the block cipher functions.
 *
 * @param[out] c block cipher operation context
 * @param[in] key1 key used for the block cipher operation, expected size
 *                16, 24 or 32 bytes
 * @param[in] key2 second key used for the XTS operation, expected
 *                 size 16, 24 or 32 bytes, must be equal to \p key1 size
 * @param[in] iv initialization vector, size must be 16 bytes
 * @param[in] mode AES mode, expected mode BLKCIPHER_MODEID_{CBC/ECB/OFB/CFB/CTR/XTS}
 * @param[in] dir expected SX_AES_ENCRYPT or SX_AES_DECRYPT
 * @param[in] params miscellaneous parameters like temporal redundancy activation
 * @return ::SX_OK
 * @return ::SX_ERR_INVALID_KEYREF
 * @return ::SX_ERR_INVALID_KEY_SZ
 * @return ::SX_ERR_INCOMPATIBLE_HW
 ...
 * @param[in] aadsz size, in bytes, of the additional authenticated data(AAD)
 * @param[in] datasz size, in bytes, of the data to be processed
 * @param[in] mode AEAD mode, expected mode AEAD_MODEID_CCM or AEAD_MODEID_GCM
 * @param[in] dir expected SX_AES_ENCRYPT or SX_AES_DECRYPT
 * @param[in] params miscellaneous parameters like temporal redundancy activation
 * @return ::SX_OK
 * @return ::SX_ERR_INVALID_KEYREF
...
int sx_aead_create_aes_generic(struct sxaead *c,
    const struct sxkeyref *key, const char *nonce_or_iv, size_t noncesz,
    size_t tagsz, size_t aadsz, size_t datasz, const uint32_t dir,
    const char mode, const struct sxaesparams *params);
