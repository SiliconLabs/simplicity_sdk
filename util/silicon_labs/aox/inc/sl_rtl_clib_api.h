...
#define SL_RTL_ROLE_INITIATOR 0
#define SL_RTL_ROLE_REFLECTOR 1
#define SL_RTL_INITIATOR_CALIB_STEP_BYTE_LEN 8
#define SL_RTL_REFLECTOR_CALIB_STEP_BYTE_LEN 6
#define SL_RTL_PBR_STEP_BYTE_LEN 12
#define SL_RTL_RTT_STEP_BYTE_LEN 9
#define SL_RTL_PCT_IQ_BIT_LEN  12
#define SL_RTL_STEP_MODE_BIT_LEN 8
#define SL_RTL_TONE_ARRAY_LEN 2

typedef struct sl_rtl_cs_parameters sl_rtl_cs_parameters;
typedef struct sl_rtl_cs_config sl_rtl_cs_config;
typedef struct sl_rtl_cs_log_params sl_rtl_cs_log_params;
typedef struct sl_rtl_cs_algorithm_param sl_rtl_cs_algorithm_param;
typedef struct sl_rtl_cs_step_data sl_rtl_cs_step_data;
typedef struct sl_rtl_cs_subevent_data sl_rtl_cs_subevent_data;
typedef struct sl_rtl_cs_procedure sl_rtl_cs_procedure;
typedef struct mode_calib_data mode_calib_data;
typedef struct mode_rtt_data mode_rtt_data;
typedef struct mode_pbr_data mode_pbr_data;
typedef struct tone tone;

enum sl_rtl_cs_algo_mode{
  SL_RTL_CS_ALGO_MODE_REAL_TIME_BASIC = 0, ///< Medium filtering, medium response, medium CPU cost. Suitable for real-time tracking.
  SL_RTL_CS_ALGO_MODE_STATIC_HIGH_ACCURACY = 1, ///< High filtering, high CPU cost, high accuracy. Suitable for static high-accuracy use cases.

  SL_RTL_CS_ALGO_MODE_LAST ///< Placeholder
};

enum sl_rtl_cs_mode{
  SL_RTL_CS_MODE_PBR = 0, ///< Round-trip phase based estimation (PBR)
  SL_RTL_CS_MODE_RTT = 1, ///< Round-trip time based estimation (RTT)

  SL_RTL_CS_MODE_LAST ///< Placeholder
};
...
PACKSTRUCT(struct sl_rtl_cs_parameters {
  uint32_t max_number_of_frequencies;              ///< Maximum number of frequencies. Affects amount of memory allocated by the RTL library.
  uint32_t delta_f;                                ///< Channel frequency jump in Hz
});

PACKSTRUCT(struct sl_rtl_cs_config {
  ...
  uint32_t main_mode_repetition;      ///< Number of main modes repeated from previous subevent (0, 1, 2, 3)
  ...
  uint8_t cs_sync_phy;
  enum sl_rtl_cs_rtt_type rtt_type;
  uint16_t acl_connection_interval;
});

enum sl_rtl_cs_algorithm_param_type {
...
union sl_rtl_cs_algorithm_param_value {
...
PACKSTRUCT(struct sl_rtl_cs_algorithm_param {
  enum sl_rtl_cs_algorithm_param_type type;
  union sl_rtl_cs_algorithm_param_value value;
});

PACKSTRUCT(struct mode_calib_data {
...
PACKSTRUCT(struct mode_rtt_data {
  uint8_t packet_quality;
  uint8_t packet_nadm;
  int8_t packet_rssi;
  int16_t packet_rtt;
...
PACKSTRUCT(struct tone {
  int32_t pct_i: SL_RTL_PCT_IQ_BIT_LEN;
  int32_t pct_q: SL_RTL_PCT_IQ_BIT_LEN;
  uint8_t quality_indicator;
});

PACKSTRUCT(struct mode_pbr_data {
  uint8_t antenna_permutation_index;
  tone tones[SL_RTL_TONE_ARRAY_LEN];
});

PACKSTRUCT(struct sl_rtl_cs_step_data {
  enum sl_rtl_cs_mode step_mode: SL_RTL_STEP_MODE_BIT_LEN;
  uint8_t step_channel;
  uint8_t step_data_length;

  union {
    mode_calib_data mode_calib_step;
    mode_rtt_data mode_rtt_step;
    mode_pbr_data mode_pbr_step;
  };
});

PACKSTRUCT(struct sl_rtl_cs_subevent_data {
  uint64_t   subevent_timestamp_ms;  ///< Relative millisecond-resolution timestamp for this subevent. Optional, use 0 if not available.
...
PACKSTRUCT(struct sl_rtl_cs_procedure {
...
enum sl_rtl_distance_estimate_type {
  SL_RTL_DISTANCE_ESTIMATE_TYPE_FILTERED = 0,
  SL_RTL_DISTANCE_ESTIMATE_TYPE_RSSI,
  //SL_RTL_DISTANCE_ESTIMATE_TYPE_RAW, (FUTURE: unfiltered distance)

  SL_RTL_DISTANCE_ESTIMATE_TYPE_LAST ///< Placeholder
};

enum sl_rtl_distance_estimate_confidence_type {
  SL_RTL_DISTANCE_ESTIMATE_CONFIDENCE_TYPE_LIKELINESS = 0,
  SL_RTL_DISTANCE_ESTIMATE_CONFIDENCE_TYPE_BIT_ERROR_RATE, ///< Bit error rate is only supported for RTT with RTT types SS_32BIT and SS_96BIT
  //SL_RTL_DISTANCE_ESTIMATE_CONFIDENCE_TYPE_DEVIATION, (FUTURE: metric deviation)
  //SL_RTL_DISTANCE_ESTIMATE_CONFIDENCE_TYPE_INPUT_DATA_QUALITY, (FUTURE: low, medium, good)

  SL_RTL_DISTANCE_ESTIMATE_CONFIDENCE_TYPE_LAST ///< Placeholder
};

/// CS library item
typedef void* sl_rtl_cs_libitem;

PACKSTRUCT(struct sl_rtl_cs_log_params {
  struct timespec timestamp;      ///< Epoch time of the RTL library initiatilization in seconds
  uint64_t timestamp_ms;          ///< Monotonic time of the RTL library initiatilization in milliseconds, must come from the same clock as subevent_timestamp_ms in sl_rtl_cs_subevent_data.
});
 ...
 * sets medium filtering. For further description of the modes, see the
 * documentation of ::sl_rtl_cs_algo_mode.
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_set_algo_mode(sl_rtl_cs_libitem* item, enum sl_rtl_cs_algo_mode mode);

/**************************************************************************//**
 * Set the estimation cs_mode.
 *
 * @param[in] item Pointer to the initialized CS libitem
 * @param[in] mode cs_mode as ::sl_rtl_cs_mode
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *
 * Set the desired cs_mode. For example, ::SL_RTL_CS_MODE_PBR uses round-trip
 * phase based cs_mode.
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_set_cs_mode(sl_rtl_cs_libitem* item, enum sl_rtl_cs_mode cs_mode);

/**************************************************************************//**
 * Set the estimation cs_mode.
 *
 * @param[in] item Pointer to the initialized CS libitem
 * @param[in] mode parameters as ::sl_rtl_cs_mode
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *
 * Set the parameters for the estimator(s).
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_set_cs_parameters(sl_rtl_cs_libitem* item, sl_rtl_cs_parameters* parameters);
...
/**************************************************************************//**
 * Set CS algorithm parameters. The function can be called any time before or during
 * the processing calls.
 *
 * @param[in] item Pointer to the initialized and configured CS libitem
 * @param[in] parameters Data structure with the CS algorithm parameter to be set
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_set_cs_algorithm_param(sl_rtl_cs_libitem* item, const sl_rtl_cs_algorithm_param* param);
 ...
 *
 * Calculate the distance estimate from the given procedure data. The function
 * can be called with multiple procedures. Use sl_rtl_cs_get_distance to fetch
 * the latest distance estimate.
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_process(sl_rtl_cs_libitem* item, uint8_t procedure_count, sl_rtl_cs_procedure* procedure_data);
 ...
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_get_distance_estimate(sl_rtl_cs_libitem* item, enum sl_rtl_distance_estimate_type estimate_type, float* distance);
 ...
 * @param[out] distance_likeliness Confidence between 0.0 - 1.0 (unlikely - likely) of the estimated distance
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_get_distance_estimate_confidence(sl_rtl_cs_libitem* item, enum sl_rtl_distance_estimate_confidence_type confidence_type, float* confidence);

/**************************************************************************//**
 * Enable logging for the given CS libitem.
 *
 * @param[in] item Pointer to the CS libitem
 * @param[in] item Pointer to the log initialisation params
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_cs_log_enable(sl_rtl_cs_libitem* item, sl_rtl_cs_log_params* log_params);
 ...
 * 5. Deinitialize logging. This will disable logging for all the remaining RTL libitems.
 */
...
typedef struct {
  sl_rtl_log_callback_function log_callback_function;                               ///< Callback function for the logging
  char sdk_version[SL_RTL_LOG_SDK_VERSION_CHAR_ARRAY_MAX_SIZE];                     ///< Unique version string
  char command_line_options[SL_RTL_LOG_COMMAND_LINE_OPTIONS_CHAR_ARRAY_MAX_SIZE];   ///< Optional field for debugging purposes. The command line options include the command name.
} sl_rtl_log_configure_params;
 ...
 *
 * @param[in] item Pointer to the callback function
 * @return ::SL_RTL_ERROR_SUCCESS if successful
 *****************************************************************************/
enum sl_rtl_error_code sl_rtl_log_configure(sl_rtl_log_configure_params *log_configure_params);
