...
sl_zigbee_ezsp_status_t spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_FATAL;
...
sl_zigbee_ezsp_status_t halNcpHardResetReqBootload(bool requestBootload)
{
  ...
  if (!waitForHostInterruptWithMsTimeout(STARTUP_TIMEOUT_MS)) {
    ...
    unlockSemaphore();
    return SL_ZIGBEE_EZSP_SPI_ERR_STARTUP_TIMEOUT;
  }
...
void halNcpSendRawCommand(void)
{
  ...
  spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_FATAL;
  ...
  if (payloadLength > SPIP_MAX_PAYLOAD_FRAME_LENGTH) {
    spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_SL_ZIGBEE_EZSP_COMMAND_OVERSIZED;
  } else {
    spipStatus = SL_ZIGBEE_EZSP_SUCCESS; //the command can now be marked as successful
    for (i = 0; i < length; i++) {
...
static void endNcpPoll(sl_zigbee_ezsp_status_t status)
{
  spipStatus = status;
...
sl_zigbee_ezsp_status_t halNcpPollForResponse(void)
{
  ...
  if (spipStatus == SL_ZIGBEE_EZSP_SPI_ERR_SL_ZIGBEE_EZSP_COMMAND_OVERSIZED) {
    endNcpPoll(SL_ZIGBEE_EZSP_SPI_ERR_SL_ZIGBEE_EZSP_COMMAND_OVERSIZED);
    return spipStatus;
  }

  spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_FATAL; //start from a fatal state
  halNcpSpipErrorByte = 0xFF;

  // check for nHOST_INT falling edge, keep waiting if not asserted
  // other hosts poll once and return SL_ZIGBEE_EZSP_SPI_WAITING_FOR_RESPONSE if the
  ...
  if (!waitForHostInterruptWithMsTimeout(WAIT_SECTION_TIMEOUT_MS)) {
    ...
    endNcpPoll(SL_ZIGBEE_EZSP_SPI_ERR_WAIT_SECTION_TIMEOUT);
    return spipStatus;
  }
  ...
  if (spipByte < 0x05) { //other error conditions
    ...
    if (spipByte == SPIP_NCP_WAS_RESET_RESPONSE) {
      ...
      spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_OVERSIZED_SL_ZIGBEE_EZSP_FRAME;
    } else if (spipByte == 0x02) {
      spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_ABORTED_TRANSACTION;
    } else if (spipByte == 0x03) {
      spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_MISSING_FRAME_TERMINATOR;
    } else if (spipByte == 0x04) {
      spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_UNSUPPORTED_SPI_COMMAND;
    }
    ...
    if (halNcpSpipBuffer[1] > SPIP_MAX_PAYLOAD_FRAME_LENGTH) {
      endNcpPoll(SL_ZIGBEE_EZSP_SPI_ERR_SL_ZIGBEE_EZSP_RESPONSE_OVERSIZED);
      return spipStatus;
    }
    for (i = 2; i < halNcpSpipBuffer[1] + 2; i++) {
      halNcpSpipBuffer[i] = spiWriteRead(0xFF); //rx the message
    }
    spipStatus = SL_ZIGBEE_EZSP_SUCCESS;
  ...
  if ( spipStatus != SL_ZIGBEE_EZSP_SPI_ERR_FATAL
       && spiWriteRead(0xFF) != SPIP_FRAME_TERMINATOR) {
    spipStatus = SL_ZIGBEE_EZSP_SPI_ERR_NO_FRAME_TERMINATOR;
  }
...
bool halNcpVerifySpiProtocolVersion(void)
{
  sl_zigbee_ezsp_status_t status = SL_ZIGBEE_EZSP_SPI_WAITING_FOR_RESPONSE;
