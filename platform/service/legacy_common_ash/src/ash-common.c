...
{
  sl_zigbee_ezsp_status_t status = SL_ZIGBEE_EZSP_ASH_IN_PROGRESS;
  ...
  switch (byte) {
    ...
    case ASH_CAN:             // cancel frame without an error
      status = SL_ZIGBEE_EZSP_ASH_CANCELLED;
      break;
    ...
    case ASH_XOFF:
      if (!ashReadConfig(rtsCts)) {
        status = SL_ZIGBEE_EZSP_ASH_ERROR_XON_XOFF;
      }
      break;
#endif
  ...
  ashDecodeInProgress = (status == SL_ZIGBEE_EZSP_ASH_IN_PROGRESS);
  return status;
}
