...
static const uint32_t powers10[9] = {
  1000000000,
  100000000,
  10000000,
  1000000,
...
#define flushBuffer()                                             \
  do { count = localBufferPointer - localBuffer;                  \
       if (flushHandler(0, localBuffer, count) != SL_STATUS_OK) { \
         goto fail; }                                             \
       total += count;                                            \
       localBufferPointer = localBuffer;                          \
    ...
    if (next != '%' && !(stillInsideFormatSpecifier)) {
      ...
      switch (*string) {
        ...
        case 's': {
          ...
          ;
          if (flushHandler(0, arg, len) != SL_STATUS_OK) {
            goto fail;
          }
          total += len;
          break;
                                         ...
                                         uint16_t count)
{
  ...
  sl_status_t status;
  uint16_t i;
  status = sli_legacy_serial_write_string(0, prefix);
  if (status != SL_STATUS_OK) {
    return status;
  }
  for (i = 0; i < count; i++) {
    status = sli_legacy_serial_printf(0,
                                      "%x%s",
                                      bytes[i],
                                      (i < count - 1
                                       ? " "
...
{
  (void) port;
  sl_status_t result = sl_legacy_serial_print_bytes(0, prefix, bytes, count);

  if (result != SL_STATUS_OK) {
    return result;
  }

  return sli_legacy_serial_printf_line(0, "");
}

sl_status_t sli_legacy_serial_print_carriage_return(uint8_t port)
{
  (void) port;
  return sli_legacy_serial_printf(0, "\r\n");
}
...
sl_status_t sli_legacy_serial_printf_var_arg(uint8_t port, const char *formatString, va_list ap)
{
  ...
  if (!sli_util_printf_internal(sli_legacy_serial_write_data, 0, formatString, ap)) {
    stat = SL_STATUS_FAIL;
  }
...
sl_status_t sli_legacy_serial_printf_line(uint8_t port, const char *formatString, ...)
{
  ...
  stat = sli_legacy_serial_printf_var_arg(0, formatString, ap);
  va_end(ap);
  sli_legacy_serial_print_carriage_return(0);
  return stat;
}
...
sl_status_t sli_legacy_serial_write_hex(uint8_t port, uint8_t dataByte)
{
  ...
  return sli_legacy_serial_write_data(0, hex, 2);
}
