...
class Calc_Modulator_Sol(CALC_Modulator_Ocelot):
    ...
    def buildVariables(self, model):
        ...
        self._addModelVariable(model, 'softm_tx_grp_delay_us', float, ModelVariableFormat.DECIMAL,
                               desc='softmodem TX group delay in us')
        self._addModelVariable(model, 'interp1_tx_grp_delay_us', float, ModelVariableFormat.DECIMAL,
                               desc='interpolation 1 TX group delay in us')
        self._addModelVariable(model, 'interp2_tx_grp_delay_us', float, ModelVariableFormat.DECIMAL,
                               desc='interpolation 2 TX group delay in us')
    ...
    def calc_tx_delay(self, model):
        if model.vars.modulator_select.value == model.vars.modulator_select.var_enum.IQ_MOD:  # : IQ modulator for OFDM
            ...
            interp1_taps = 5 *interp1_ratio  # : FIR Polyphase filter with 5 taps
            interp1_delay = interp1_taps/2  #Even symmetrical filter
            interp1_grp_delay_us = interp1_delay / interp1_rate * 1e6
            ...
            model.vars.softm_tx_grp_delay_us.value = softmodem_grp_delay_us
            model.vars.interp1_tx_grp_delay_us.value = interp1_grp_delay_us
            model.vars.interp2_tx_grp_delay_us.value = interp2_grp_delay_us
            model.vars.tx_grp_delay_us.value = 0.0
        else:
            super().calc_tx_delay(model)