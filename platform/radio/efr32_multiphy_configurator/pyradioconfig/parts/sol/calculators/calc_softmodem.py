...
class calc_softmodem_sol(ICalculator):
    ...
    def buildVariables(self, model):
        ...
        self._addModelVariable(model, 'ofdm_preemph_sub_filter_number', int, ModelVariableFormat.DECIMAL, desc='Number of subcarrier to filter (to improve ARIB margin)')
    ...
    def calc_softmodem_sunofdm_version_regs(self, model):
        ...
        self._reg_write(model.vars.SUNOFDM_VREGS_MINORVERSION, 2)
    ...
    def calc_softmodem_sunofdm_ant_regs(self, model):
        ...
        softmodem_antdivmode = model.vars.softmodem_antdivmode.value

        # Handle supported softmodem antdiv modes here
        if softmodem_antdivmode == model.vars.softmodem_antdivmode.var_enum.ANTENNA1:
            sunofdm_ant_antsel = 1
        elif softmodem_antdivmode == model.vars.softmodem_antdivmode.var_enum.ANTSELFIRST:
            sunofdm_ant_antsel = 2
        elif softmodem_antdivmode == model.vars.softmodem_antdivmode.var_enum.ANTSELCORR:
            sunofdm_ant_antsel = 3
        elif softmodem_antdivmode == model.vars.softmodem_antdivmode.var_enum.ANTSELRSSI:
    ...
    def calc_softmodem_sunofdm_preemphasis_regs(self, model):
        # https://jira.silabs.com/browse/MCUW_RADIO_CFG-2374

        softmodem_modulation_type = model.vars.softmodem_modulation_type.value

        if softmodem_modulation_type == model.vars.softmodem_modulation_type.var_enum.SUN_OFDM:

            ofdm_preemph_sub_filter_number = model.vars.ofdm_preemph_sub_filter_number.value

            if ofdm_preemph_sub_filter_number:
                preemph = 1  # Enable preemphasis filtering on the payload
                preemphstfen = 0  # Disable preemphasis filtering for the STF
                mcs_table = 255  # Filter whatever the MCS
            else:
                preemph = 0
                preemphstfen = 0
                mcs_table = 0

            self._reg_write(model.vars.SUNOFDM_PREEMPH_PREEMPHEN, preemph)
            self._reg_write(model.vars.SUNOFDM_PREEMPH_PREEMPHSTFEN, preemphstfen)
            self._reg_write(model.vars.SUNOFDM_PREEMPH_MCSTABLE, mcs_table)

        else:
            self._reg_do_not_care(model.vars.SUNOFDM_PREEMPH_PREEMPHEN)
            self._reg_do_not_care(model.vars.SUNOFDM_PREEMPH_PREEMPHSTFEN)
            self._reg_do_not_care(model.vars.SUNOFDM_PREEMPH_MCSTABLE)

    def calc_softmodem_sunofdm_preemphasis_coeffs(self, model):
        # https://jira.silabs.com/browse/MCUW_RADIO_CFG-2367
        ...
        if softmodem_modulation_type == model.vars.softmodem_modulation_type.var_enum.SUN_OFDM:
            preemphasis_coeff = self.return_preemphasis_coeff(ofdm_subcarrier_count, ofdm_preemph_sub_filter_number)

            for i in range(52):
                field = getattr(model.vars, f'SUNOFDM_PREEMPHCOGR{i//4}_COEFF{i}')
                self._reg_write(field, preemphasis_coeff[i])

        else:
            for i in range(52):
                field = getattr(model.vars, f'SUNOFDM_PREEMPHCOGR{i//4}_COEFF{i}')
                self._reg_do_not_care(field)

    def return_preemphasis_coeff(self, n_subs, n_filtered_subs):
        n_subs = n_subs//2  # The filtering values are applied symmetrically

        # Lower subcarrier power by 1 * index dB (filtered[0]: -1dB, filtered[1]: -2dB...)
        filtered_subs_buffer = [int(128 * 10 ** (-(i+1)/20)) for i in range(n_filtered_subs)]
        unfiltered_subs_buffer = [128] * (n_subs - n_filtered_subs)  # 128: no filter
        preemphasis_coeff = filtered_subs_buffer[::-1] + unfiltered_subs_buffer

        # Calculate filtered and unfiltered channel power
        power_wo_filter = (n_subs * 128 ** 2)
        power_w_filter = sum(filter_value ** 2 for filter_value in preemphasis_coeff)

        # Compensate for filter loss
        for i in range(n_subs):
            preemphasis_coeff[i] = int(preemphasis_coeff[i] * (power_wo_filter/power_w_filter) ** 0.5)

        # Fill the buffer with the coefficient of unused subs
        preemphasis_coeff += [128] * (52 - n_subs)

        return preemphasis_coeff
    ...
    def calc_softmodem_sunfsk_version_regs(self, model):
        self._reg_write(model.vars.SUNFSK_VREGS_MAJORVERSION, 1)
    ...
    def calc_sunfsk_mode_reg(self, model):
        ...
        # Should become 'do_not_care' once the firmware handles MAJORVERSION = 1
    ...
    def calc_sunofdm_antdivdly_reg(self, model):
        ...
        if softmodem_modulation_type == model.vars.softmodem_modulation_type.var_enum.SUN_OFDM and \
                softmodem_antdivmode != model.vars.softmodem_antdivmode.var_enum.DISABLE:
            ofdm_option = model.vars.ofdm_option.value
            ofdm_stf_length = model.vars.ofdm_stf_length.value
            ...
            self._reg_write(model.vars.SUNOFDM_ANTDIVDLY2_AGCCONVDLYRED, agcconvdlyred)
            ...
            self._reg_do_not_care(model.vars.SUNOFDM_ANTDIVDLY2_AGCCONVDLYRED)

    def calc_softmodem_sunfsk_colldet_regs(self, model):
        self._reg_write(model.vars.SUNFSK_COLLDET_COLLDET, 0)
        self._reg_write(model.vars.SUNFSK_COLLDET_COLLDETTHR, 10)

    def calc_softmodem_sunfsk_txcfg1_regs(self, model):
        self._reg_write(model.vars.SUNFSK_TXCFG1_TXHCSERRORS, 0)  # Disabled
        self._reg_write(model.vars.SUNFSK_TXCFG1_TXFCSERRORS, 0)  # Disabled

    def calc_softmodem_sunfsk_ant_regs(self, model):
        self._reg_write(model.vars.SUNFSK_ANT_ANTDIV, 0)
        self._reg_write(model.vars.SUNFSK_ANT_ANTSEL, 0)

    def calc_softmodem_sunfsk_afc_regs(self, model):
        ...
        if softmodem_modulation_type == model.vars.softmodem_modulation_type.var_enum.SUN_FSK:
            ...
            if afc_correction == 2:  # For now we always choose this
                afc_mixer_ratio_att = 14
                afc_mixer_ratio_gain = round((2 ^ (3+afc_mixer_ratio_att) * lodiv_actual * bitrate) / xtal_frequency_hz)
            else:
                afc_mixer_ratio_att = 13
                afc_mixer_ratio_gain = round((2 ^ (4+afc_mixer_ratio_att) * bitrate) / fdec0_out_hz)
        ...
        self._reg_write(model.vars.SUNFSK_AFC_AFCMIXERRATIOGAIN, int(afc_mixer_ratio_gain), check_saturation=True)
        self._reg_write(model.vars.SUNFSK_AFC_AFCMIXERRATIOATT, int(afc_mixer_ratio_att), check_saturation=True)
        self._reg_write(model.vars.SUNFSK_AFC_AFCCORRECTION, int(afc_correction))
