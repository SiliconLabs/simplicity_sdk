...
uint16_t sl_zigbee_counters[SL_RAIL_UTIL_COEX_EVENT_COUNT];

void sl_zigbee_clear_counters(void)
{
  memset(&sl_zigbee_counters, 0, sizeof(sl_zigbee_counters));
}

void sl_rail_util_coex_counter_on_event(sl_rail_util_coex_event_t event)
{
  sl_zigbee_counters[event] += 1;
}
#else
extern const char * titleStrings[];
extern uint16_t sl_zigbee_counters[];
extern void sl_zigbee_clear_counters(void);
#endif

static void printCounter(uint8_t id)
{
  responsePrintContinue("%s: %u", titleStrings[id], sl_zigbee_counters[id]);
}

static void printLastCounter(uint8_t id)
{
  responsePrintEnd("%s: %u", titleStrings[id], sl_zigbee_counters[id]);
}
