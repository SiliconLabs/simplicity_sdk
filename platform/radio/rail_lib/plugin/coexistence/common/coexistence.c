...
{
  (*coexHalCallbacks->enableGpioInt)(gpioHandle, true);
}

static void disableGpioInt(COEX_GpioHandle_t gpioHandle)
{
  (*coexHalCallbacks->enableGpioInt)(gpioHandle, false);
}
...
void coexEventCallback(COEX_Events_t events)
{
...
{
  COEX_Events_t gntEvents = COEX_EVENT_NONE;
  ...
  if (getGpioOut(reqHandle, false)) {    // GRANT phase
    ...
    if (gntChanged) {
      ...
      if (!newGnt) {
        gntEvents = COEX_EVENT_GRANT_RELEASED;
        ...
        gntEvents = COEX_EVENT_GRANT_ASSERTED;
      }
    }
  } else {                          // REQUEST phase
  ...
  if (gntEvents != COEX_EVENT_NONE) {
    coexEventCallback(gntEvents);
  }
...
static void coexRadioHoldOffPowerUp(void)
{
  if (!coexCfg.radioOn) {
    ...
    if (rhoHandle != NULL) {
      ...
      enableGpioInt(rhoHandle); //enable RHO interrupt
    }
  }
}
  ...
  if (myReq) {                    // want to assert REQUEST
    if (force || !exReq) {        // can assert REQUEST
      if (!getGpioOut(reqHandle, false)) {
        // coexEventCallback called here to avoid coex_simulation_test error
        // when it is put in the end of this function
        coexEventCallback(txrxReq);
        checkGnt = isGpioInSet(gntHandle, false);
        if (checkGnt) {
          coexEventCallback(COEX_EVENT_GRANT_ASSERTED | COEX_EVENT_REQUEST_ASSERTED);
        } else {
          coexEventCallback(COEX_EVENT_REQUEST_ASSERTED);
        }
        // Assume request denied until request is granted
        coexCfg.requestDenied = true;
      }
      enableGpioInt(gntHandle);
      bool highPriority = ((combinedReqState & COEX_REQ_HIPRI) != 0U);
