...
void scheduleNextTx(void)
{
  ...
  if ((txCount > 0 && currentAppMode() != TX_SCHEDULED_N_PACKETS)
    ...
    if (enableRandomTxDelay) {
      ...
      sl_rail_set_timer(railHandle, (uint32_t) fTxDelay, SL_RAIL_TIME_DELAY, &railtest_TimerExpired);
    } else {
      sl_rail_set_timer(railHandle, continuousTransferPeriod * 1000, SL_RAIL_TIME_DELAY, &railtest_TimerExpired);
    }

    if (isHFXOCompensationSystematic) {
      // Always start thermistor when systematic mode is engaged
      sl_rail_start_thermistor_measurement(railHandle);
    }
    ...
    if (txCount) {
      ...
      sl_rail_scheduled_tx_config_t scheduledTxOptions = {
        ...
        .when = ((txScheduledTime != 0U) ? txScheduledTime : txStartTime)
                + continuousTransferPeriod * 1000U,
...
void radioTransmitGeneric(uint32_t iterations, bool scheduled, char *command)
{
  ...
  if (iterations > 0) {
    if (currentAppMode() == TX_UNDERFLOW) {
      ...
      if (scheduled) {
        setNextAppMode(TX_SCHEDULED_N_PACKETS, command);
      } else {
...
// Specifically for non scheduled transmits
void radioTransmit(uint32_t iterations, char *command)
{
  radioTransmitGeneric(iterations, false, command);
}
