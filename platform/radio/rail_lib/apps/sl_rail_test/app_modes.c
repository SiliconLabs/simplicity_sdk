...
sl_rail_tx_options_t antOptions = SL_RAIL_TX_OPTIONS_DEFAULT;
sl_rail_tx_options_t concPhyIdOptions = SL_RAIL_TX_OPTIONS_DEFAULT;
sl_rail_stream_mode_t streamMode = SL_RAIL_STREAM_PN9_STREAM;
...
static void transitionAppMode(AppMode_t nextAppMode)
{
  ...
  if (nextAppMode == TX_STREAM) {
    sl_rail_start_tx_stream(railHandle, channel, streamMode, concPhyIdOptions | antOptions);
  } else if (nextAppMode == DIRECT) {
    sl_rail_enable_direct_mode(railHandle, true, true);
  } else if (nextAppMode == TX_CONTINUOUS || nextAppMode == TX_N_PACKETS
             || nextAppMode == TX_SCHEDULED_N_PACKETS) {
    pendPacketTx();
