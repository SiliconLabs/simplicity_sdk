/***************************************************************************//**
 * @file
 * @brief This file handles the hardware interactions for sl_rail_test
 *******************************************************************************
 * # License
...
void appHalInit(void)
{
  ...
  // For PER test
  sl_gpio_set_pin_mode(&(sl_gpio_t){.port = SL_RAIL_TEST_PER_PORT, .pin = SL_RAIL_TEST_PER_PIN }, SL_GPIO_MODE_PUSH_PULL, true);
...
{
  (void) sl_rail_delay_us(railHandle, (sl_rail_time_t) microseconds);
}

#ifdef _SILICON_LABS_32B_SERIES_2
#define sl_hal_eusart_get_status EUSART_StatusGet
#endif
...
#endif
#ifdef SL_CATALOG_IOSTREAM_EUSART_PRESENT
  while ((sl_hal_eusart_get_status(SL_IOSTREAM_EUSART_VCOM_PERIPHERAL)
...
void disableGraphics(void)
{
  ...
  sl_gpio_set_pin_mode(&(sl_gpio_t){BSP_VCOM_ENABLE_PORT, BSP_VCOM_ENABLE_PIN }, SL_GPIO_MODE_PUSH_PULL, true);
#endif
}
...
{
  if (!inAppMode(NONE, NULL) || inRadioState(SL_RAIL_RF_STATE_TX, NULL)) {
    return;
  }

  if (inRadioState(SL_RAIL_RF_STATE_RX, NULL)) {
    sl_rail_idle(railHandle, SL_RAIL_IDLE_ABORT, false);
  }

  // Check if next channel exists
  if (sl_rail_is_valid_channel(railHandle, channel + 1)
      == SL_RAIL_STATUS_NO_ERROR) {
    channel++;
  } else {
    // Find initial channel
    channel = 0;
    while (sl_rail_is_valid_channel(railHandle, channel)
           != SL_RAIL_STATUS_NO_ERROR) {
      channel++;
    }
  }
...
{
  if (initButtonStatus) {
    #define GET_TIME_IN_MS() (sl_rail_get_time(railHandle) / 1000)
