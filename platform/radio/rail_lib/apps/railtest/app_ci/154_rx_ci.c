...
void ieee802154Config2MbpsRxTimeout(sl_cli_command_arg_t *args)
{
  ...
  RAIL_Status_t status = RAIL_IEEE802154_Config2MbpsRxTimeout(railHandle, timeout);
  if (status != RAIL_STATUS_NO_ERROR) {
...
void RAILCb_IEEE802154_DataRequestCommand(RAIL_Handle_t railHandle)
{
  ...
  if (ieee802154EnhAckEnabled) {
    ...
    if (enhAck) {
      ...
      if (RAIL_IEEE802154_WriteEnhAck(railHandle, pPkt, (uint16_t)pktOffset)
...
void ieee802154ConfigRxChannelSwitching(sl_cli_command_arg_t *args)
{
  ...
  RAIL_IEEE802154_RxChannelSwitchingCfg_t *pConfig = &config;
  ...
  RAIL_Status_t status = RAIL_IEEE802154_ConfigRxChannelSwitching(railHandle, pConfig);
