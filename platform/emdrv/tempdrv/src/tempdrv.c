...
#else
// On Series-2/3 devices the temperature code is normal. (high value = high temperature)
...
#define EMU_GET_TEMPERATURE  sl_hal_emu_get_temperature
#else
#define EMU_GET_INTERRUPTS   EMU_IntGetEnabled
#define EMU_INT_CLEAR        EMU_IntClear
#define EMU_INT_ENABLE       EMU_IntEnable
#define EMU_INT_DISABLE      EMU_IntDisable
#define EMU_GET_TEMPERATURE  EMU_TemperatureGet
#endif // _SILICON_LABS_32B_SERIES_3
...
{
  uint32_t flags = EMU_GET_INTERRUPTS();
  ...
  if (flags & DECREASING_TEMPERATURE_FLAG) {
    ...
    EMU_INT_CLEAR(DECREASING_TEMPERATURE_FLAG);
    ...
    EMU_INT_CLEAR(INCREASING_TEMPERATURE_FLAG);
  }
...
{
  EMU_INT_CLEAR(EMU_IF_TEMPLOW | EMU_IF_TEMPHIGH);
  EMU_INT_DISABLE(EMU_IF_TEMPLOW | EMU_IF_TEMPHIGH);
}
...
static void updateInterrupts(void)
{
  ...
  if (highCallback->callback != NULL) {
    ...
    EMU_INT_ENABLE(EMU_IEN_TEMPLOW);
  } else {
    EMU_INT_DISABLE(EMU_IEN_TEMPLOW);
  }
  if (lowCallback->callback != NULL) {
    ...
    EMU_INT_ENABLE(EMU_IEN_TEMPHIGH);
  } else {
    EMU_INT_DISABLE(EMU_IEN_TEMPHIGH);
  }
#else
  // On Series-2/3 devices the temperature code is normal (high value = high temperature)
  if (highCallback->callback != NULL) {
...
int8_t TEMPDRV_GetTemp(void)
{
  ...
  tempCelsius = EMU_GET_TEMPERATURE();
