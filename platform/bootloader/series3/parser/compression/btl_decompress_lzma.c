...
#define OUTPUT_BUFFER_SIZE          (2048UL)

// Configurable alignment size, it should be less than the OUTPUT_BUFFER_SIZE
#define ALIGNMENT_SIZE OUTPUT_BUFFER_SIZE
// Aligns x down to the nearest multiple of ALIGNMENT_SIZE
#define ALIGN_DOWN(x) ((x) & ~(ALIGNMENT_SIZE - 1))
                                  ...
                                  bool                              finish)
{
  ...
  while (status == LZMA_STATUS_NOT_FINISHED) {
    ...
    size_t alignedAvailableData = ALIGN_DOWN(availableData);
    if ( alignedAvailableData > 0) {
      if ((ctx->regionId == 0) && (callbacks->bootloaderCallback != NULL)) {
        callbacks->bootloaderCallback(ctx->programmingAddress,
                                      outputBuffer,
                                      alignedAvailableData,
                                      0,
                                      ctx);
        ...
        uint32_t addOffset;
        addOffset = ctx->programmingAddress - ctx->currentUpdateMemorySection.updateMemorySection.targetAddr;
      ...
      ctx->programmingAddress += alignedAvailableData;
    }
                             ...
                             const BootloaderParserCallbacks_t *callbacks)
{
  ...
  while (length) {
    ...
    uint8_t *dataArray = (uint8_t *)data + processedBytes;
    size_t dataOffset = 0UL;

    if (firstCallInProgTag) {
      BTL_ASSERT(chunkSize >= 17UL);
      ...
      // 5 byte header + 8 byte file length should be skipped
      dataOffset = 13UL;
    }

    size_t remainingInputBytes = chunkSize - dataOffset;
                            ...
                            const BootloaderParserCallbacks_t *callbacks)
{
  ...
  if (outputBufferPos > 0) {
    ...
    size_t paddedSize = (outputBufferPos + (PAD_SIZE - 1)) & ~(PAD_SIZE - 1);
    for (size_t i = outputBufferPos; i < paddedSize; i++) {
