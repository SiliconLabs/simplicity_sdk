# Internal Storage Bootloader (Single Image on 3200 KB Device)

This sample configuration of the Gecko bootloader configures the bootloader to use the internal main flash to store firmware update images. The storage configuration is set up to store a single firmware update image at a time, in a single storage slot. The storage slot is configured to start at address 0x8190000, and have a size of 1552 kB. This can be configured on the *"Bootloader Storage Slot Setup"*.
