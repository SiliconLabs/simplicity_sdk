...
static uint8_t saturation_percent = 0;
static uint8_t hue_percent = 0;
...
static void set_device_name(uuid_128 *uuid)
{
  ...
  snprintf(name, NAME_BUF_LEN, "Switch HSL %02x%02x",
...
void app_button_press_cb(uint8_t button, uint8_t duration)
{
  ...
  if (button == BUTTON_PRESS_BUTTON_0) {
    switch (duration) {
      ...
      case APP_BUTTON_PRESS_DURATION_SHORT: {
        ...
        sl_btmesh_hsl_client_set_lightness(lightness_percent);
      } break;
      // Handling of button press greater than 0.25s and less than 1s
      case APP_BUTTON_PRESS_DURATION_MEDIUM: {
        saturation_percent = wrap_add(saturation_percent, DECREASE, MAX_PERCENT);
        sl_btmesh_hsl_client_set_saturation(saturation_percent);
      } break;
      // Handling of button press greater than 1s and less than 5s
      case APP_BUTTON_PRESS_DURATION_LONG: {
        hue_percent = wrap_add(hue_percent, DECREASE, MAX_PERCENT);
        sl_btmesh_hsl_client_set_hue(hue_percent);
      } break;
  ...
  switch (duration) {
    ...
    // Handling of button press greater than 0.25s and less than 1s
    case APP_BUTTON_PRESS_DURATION_MEDIUM: {
      saturation_percent = button == BUTTON_PRESS_BUTTON_0
                           ? clamp_add(saturation_percent, DECREASE, MAX_PERCENT)
                           : clamp_add(saturation_percent, INCREASE, MAX_PERCENT);
      sl_btmesh_hsl_client_set_saturation(saturation_percent);
    } break;
    // Handling of button press greater than 1s and less than 5s
    case APP_BUTTON_PRESS_DURATION_LONG: {
      hue_percent = button == BUTTON_PRESS_BUTTON_0
                    ? clamp_add(hue_percent, DECREASE, MAX_PERCENT)
                    : clamp_add(hue_percent, INCREASE, MAX_PERCENT);
      sl_btmesh_hsl_client_set_hue(hue_percent);
    } break;
    // Handling of button press greater than 5s
    case APP_BUTTON_PRESS_DURATION_VERYLONG: {
