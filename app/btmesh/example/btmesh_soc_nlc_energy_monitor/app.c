/***************************************************************************//**
 * @file
 * @brief Silicon Labs SoC Energy Monitor Example Project
 *******************************************************************************
 * # License
 ...
 ******************************************************************************/
static void app_led_on_blinking_timer(app_timer_t *handle, void *data);
...
//Increment both the whole and fractional part of simulated power consumption
static energy32_t simulated_power_differential = 0x050E;
...
void sl_btmesh_power_consumption_decrease()
{
  if (SL_BTMESH_SENSOR_ENERGY_USAGE_VALUE_IS_NOT_KNOWN == sl_btmesh_get_power_consumption()) {
    //Only allow increment while the value is unknown
    return;
  }
  energy32_t power_consumption = sl_btmesh_get_power_consumption();
  if (power_consumption > simulated_power_differential) {
    sl_btmesh_set_power_consumption(power_consumption - simulated_power_differential);
  } else {
    sl_btmesh_set_power_consumption(0);
  }
}

void sl_btmesh_power_consumption_increase()
{
  energy32_t power_consumption = sl_btmesh_get_power_consumption();
  if (SL_BTMESH_SENSOR_ENERGY_USAGE_VALUE_IS_NOT_KNOWN == power_consumption) {
    sl_btmesh_set_power_consumption(simulated_power_differential);
  } else if ((power_consumption + simulated_power_differential) < power_consumption) {
    //Overflow, set the maximum value
    sl_btmesh_set_power_consumption(0xFFFFFFFD);
  } else {
    sl_btmesh_set_power_consumption(simulated_power_differential + power_consumption);
  }
}
...
void app_button_press_cb(uint8_t button, uint8_t duration)
{
  ...
  if (duration == APP_BUTTON_PRESS_DURATION_SHORT) {
    if (button == BUTTON_PRESS_BUTTON_0) {
      ...
      sl_btmesh_power_consumption_increase();
    } else if (button == BUTTON_PRESS_BUTTON_1) {
      app_log("PB1 pressed" APP_LOG_NL);
      sl_btmesh_power_consumption_decrease();
    }
  }
}
...
void app_init(void)
{
  ...
  /////////////////////////////////////////////////////////////////////////////
  app_log("BT mesh NLC Energy Monitor initialized" APP_LOG_NL);
