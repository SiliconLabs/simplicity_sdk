/***************************************************************************//**
 * @file sl_rail_sdk_wmbus_support.h
 * @brief Wireless M-Bus Support
 *******************************************************************************
 * # License
...
#ifndef SL_RAIL_SDK_WMBUS_SUPPORT_H
#define SL_RAIL_SDK_WMBUS_SUPPORT_H
...
/**
 * @struct sl_rail_sdk_wmbus_dll_header_t
...
typedef struct {
  ...
  union {
    ...
    struct {
      sl_rail_sdk_wmbus_function_code_t function_code : 4;    /**< Control field function code */
      bool fcv_dcf : 1;                          /**< Control field fcv/dcf */
...
/**
 * @union sl_rail_sdk_wmbus_conf_word_t
 * @brief Data link layer header (first block)
 */
typedef union {
  ...
  struct {
    ...
    uint8_t content_of_message : 2;            /**< Content of message */
    ...
    sl_rail_sdk_wmbus_accessibility_t accessibility : 2; /**< Accessibility */
  } mode_0_5;                               /**< Detailed config word for mode 0 and 5 */
//add m2_3, m7 and m13 here
} sl_rail_sdk_wmbus_conf_word_t;

/**
 * @struct sl_rail_sdk_wmbus_stl_header_t
...
typedef struct {
  ...
  sl_rail_sdk_wmbus_conf_word_t conf_word; /**< Config word */
} sl_rail_sdk_wmbus_stl_header_t;

/**
 * @struct sl_rail_sdk_wmbus_ltl_header_t
 ...
 * \ref sl_rail_sdk_wmbus_get_meter_limited_acc_rx_start().
 *
 * @param[in] mode is the desired mode of the Wireless M-Bus
 */
 ...
 *
 * @return The selected mode of the Wireless M-Bus
 */
...
/**
 * Helper function for building the payload of a Wireless M-Bus frame. Adds an 8 bit
 * integer (binary) data block to a buffer.
 * @param[in] buffer The buffer to write into. Should point at the first free byte
 * @param[in] dibFunction Function field in the DIB (Data Information Block)
 * @param[in] vib Value information block containing the VIF (Value Information Field)
 * @param[in] value The value of the data block.
 * @return the number of bytes written into buffer
 */
uint8_t sl_rail_sdk_wmbus_frame_add_payload_int8(uint8_t *buffer,
                                                 sl_rail_sdk_wmbus_frame_dib_function_t dibFunction, uint8_t vib, int8_t value);

/**
 * Helper function for building the payload of a Wireless M-Bus frame. Adds a 16 bit
 * integer (binary) data block to a buffer.
 * @param[in] buffer The buffer to write into. Should point at the first free byte
 * @param[in] dibFunction Function field in the DIB (Data Information Block)
 * @param[in] vib Value information block containing the VIF (Value Information Field)
...
uint8_t sl_rail_sdk_wmbus_frame_add_payload_int16(uint8_t *buffer,
                                                  sl_rail_sdk_wmbus_frame_dib_function_t dibFunction, uint8_t vib, int16_t value);

/**
 * Helper function for building the payload of a Wireless M-Bus frame. Adds a 32 bit
 * integer (binary) data block to a buffer.
 * @param[in] buffer The buffer to write into. Should point at the first free byte
 * @param[in] dibFunction Function field in the DIB (Data Information Block)
 * @param[in] vib Value information block containing the VIF (Value Information Field)
...
uint8_t sl_rail_sdk_wmbus_frame_add_payload_int32(uint8_t *buffer,
                                                  sl_rail_sdk_wmbus_frame_dib_function_t dibFunction, uint8_t vib, int32_t value);

/**
 * Helper function for building the payload of a Wireless M-Bus frame.
 * Adds filler bytes required before encryption
 * @param[in] buffer The buffer to write into. Should point at the first free byte
 * @param[in] fillerLength Number of bytes to fill.
 * @return the number of bytes written into buffer
 */
 ...
 * 2B manufacturer ID.
 * @param[in] manuf Pointer to a 3 element char array, containing the uppercase letters
 *    of the manufacturer assigned by also known as DLMS FLAG ID
 * @return the manufacturer id (or M-field) for M-Bus addressing
 */
 ...
 * @param[in] mfield mfield value
 * @param[out] manuf pointer to a 3 element char array to write the 3 letters
 */
 ...
 *
 * @param[in] input Input data. Should be at least 16B*encryptedBlocks long.
 * @param[out] output Output data. Should be at least 16B*encryptedBlocks long.
 *    Can be the same as input.
 * @param[in,out] iv Initialization vector, 16B long
 * @param[in] length Length of the input data
 * @param[in] encryptedBlocks Number of blocks to encode. If 0, all data will be encoded
 *    based on length
 * @return the number of blocks encoded. Output length is
 *    16 times this value.
 */
uint8_t sl_rail_sdk_wmbus_frame_crypto5_encrypt(uint8_t *input, uint8_t *output, uint8_t *iv,
                                                uint16_t length, uint8_t encryptedBlocks);

/**
 * Decrypts blocks using mode5 (AES-CBC) encryption.
 *
 * @param[in] input  Input data. Should be at least length long.
 * @param[out] output Output data. Should be at least length long.
 *    Can be the same as input.
 * @param[in,out] iv Initialization vector, 16B long
 * @param[in] length Length of the input data. Should be divisible by 16
 */
void sl_rail_sdk_wmbus_frame_crypto5_decrypt(uint8_t *input, uint8_t *output, uint8_t *iv,
                                             uint16_t length);

#endif // SL_RAIL_SDK_WMBUS_SUPPORT_H
