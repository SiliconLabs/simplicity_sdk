 ...
 *
 */
uint32_t sl_zigbee_af_price_common_cluster_convert_duration_to_seconds(uint32_t startTimeUtc, uint32_t duration, uint8_t durationType);

/**
 * @brief Calculates a new UTC start time value based on the duration type parameter.
 *
 */
uint32_t sl_zigbee_af_price_common_cluster_get_adjusted_start_time(uint32_t startTimeUtc, uint8_t durationType);
