...
#define bootloadPrintln(...) sl_zigbee_af_core_println(__VA_ARGS__)
#define bootloadDebugExec(x) sl_zigbee_af_core_debug_exec(x)
...
static bool transferFile(const sl_zigbee_af_ota_image_id_t* id, uint16_t ncpUpgradeTagId);
...
static bool transferFile(const sl_zigbee_af_ota_image_id_t* id,
                         uint16_t ncpUpgradeTagId)
{
  uint8_t i = 0;
  uint8_t tagCount = 0;
  uint32_t offset[SL_ZIGBEE_AF_PLUGIN_OTA_STORAGE_COMMON_MAX_TAGS_IN_OTA_FILE];
  uint32_t endOffset[SL_ZIGBEE_AF_PLUGIN_OTA_STORAGE_COMMON_MAX_TAGS_IN_OTA_FILE];
  ...
  for (i = 0; i < SL_ZIGBEE_AF_PLUGIN_OTA_STORAGE_COMMON_MAX_TAGS_IN_OTA_FILE; i++) {
