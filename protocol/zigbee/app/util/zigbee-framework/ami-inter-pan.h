...
/**
 * @brief Create an interpan message suitable for passing to sl_zigbee_send_raw_message().
 */
sli_buffer_manager_buffer_t makeInterPanMessage(sli_zigbee_interpan_header_t *headerData,
                                                sli_buffer_manager_buffer_t payload);

/**
 * @brief Meant to be called on the message and offset values passed
 * to sl_802154_passthrough_message_handler(...).  The header is parsed and
 * the various fields are written to the sli_zigbee_interpan_header_t.  The returned
 * value is the offset of the payload in the message, or 0 if the
 * message is not a correctly formed AMI interPAN message.
 */
uint8_t parseInterPanMessage(sli_buffer_manager_buffer_t message,
                             uint8_t startOffset,
                             sli_zigbee_interpan_header_t *headerData);
