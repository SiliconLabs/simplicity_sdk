...
void RETARGET_SerialInit(void)
{
  ...
  sl_gpio_t gpio_tx, gpio_rx;
  gpio_tx.port = RETARGET_TXPORT;
  gpio_tx.pin = RETARGET_TXPIN;
  gpio_rx.port = RETARGET_RXPORT;
  gpio_rx.pin = RETARGET_RXPIN;
  sl_gpio_set_pin_mode(&gpio_tx, SL_GPIO_MODE_PUSH_PULL, 1);
  sl_gpio_set_pin_mode(&gpio_rx, SL_GPIO_MODE_INPUT_PULL, 1);
#else
#if defined(RETARGET_EUSART)
  /* To avoid false start, configure output as high */
